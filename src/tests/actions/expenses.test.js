...
beforeEach((done) => {  //setup dummy firebase data
  ...
  database.ref(`users/${uid}/expenses`).set(expensesData).then(() => done());  //store them in firebase
});
...
test('should remove expense from firebase', (done) => {
  const store = createMockStore(defaultAuthState); // make sure when test starts, mockstore has the uid attribute for us to manipulate
  ...
  store.dispatch(startRemoveExpense({ id })).then(() => {
    ...
    return database.ref(`users/${uid}/expenses/${id}`).once('value');  // grab data from firebase and make sure the data comes back is null
  }).then((snapshot) => {
    expect(snapshot.val()).toBeFalsy();   //toBeFalsy check if the data is null
    done();
  });
});
